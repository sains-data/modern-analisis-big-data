...
| Topic latihan | `transaksi-stream`, `sensor-iot`, `penjualan-agregat` |
| Python venv | `Konfigurasi-lab/.venv` (Python 3.10/3.11) |

> Semua perintah di latihan ini dijalankan dari folder **`Konfigurasi-lab/`**.
...
Script `start.sh` akan menaikkan Kafka + Kafka UI dan membuat topic latihan otomatis.
...
```
NAME                  IMAGE                          STATUS
modul8-kafka-broker   apache/kafka:3.7.0             Up (healthy)
modul8-kafka-ui       provectuslabs/kafka-ui:latest  Up
```
...
### Langkah 1.2 — Verifikasi topic latihan

Topic sudah dibuat oleh `start.sh`. Verifikasi dengan:

```bash
docker exec modul8-kafka-broker kafka-topics.sh \
  --list \
...
```bash
docker exec -it modul8-kafka-broker kafka-console-consumer.sh \
  --bootstrap-server localhost:9092 \
  --topic transaksi-stream \
  --from-beginning
```
...
```bash
docker exec -it modul8-kafka-broker kafka-console-producer.sh \
  --bootstrap-server localhost:9092 \
  --topic transaksi-stream
```
